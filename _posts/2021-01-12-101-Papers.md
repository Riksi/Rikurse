...
In the DDPM paper the covariance of the reverse process, $\Sigma_{\theta}\left(\xt, t\right)$ is modelled as a diagonal covariance $\sigma_t\II$ and it set as a hyperparameter.

Here they try two choices for $\sigma_t$, $\bt$ and $\tbt$. For large values of $t$ these two are roughly equal and for larger $T$ they converge sooner so it might seem that the choice of $\sigma_t$ is not that important.
...
The method proposed in Improved Diffusion is to parameterise the variance as an interpolation between $\bt$ and $\tbt$ with learned parameter $v$

$$\Sigma_{\theta}\left(\xt, t\right) = \exp\left(v\log\bt + (1 - v)\log \tbt\right)$$
...
$$L_\text{vlb} = \E_{t \sim p_t}\left[\frac{L_t}{p_t}]
