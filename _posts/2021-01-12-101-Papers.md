...
It is necessary for the wave function to be anti-symmetric to the interchange pairs.

The goal is then to learn $\phi\left(\mathbf{r}_a\right)$ which is done using as features the following:

- Per electron features - position $\mathbf{r}_a$ and distance to nucleus $\left\lvert\mathbf{R} - \mathbf{r}_a\right\rvert$
