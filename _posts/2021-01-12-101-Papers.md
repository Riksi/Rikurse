...
#### Page 2
- AlphaGo Fan, which defeated European champion Fan Hui, had a policy neural network trained to predict expert moves and a value neural net trainer to predict winners of games that policy netword played against itself 
...
- Neural net $f_\theta$ takes as input a raw board representation of the position $s$ and its history and outputs $(\mathbf{p}, v)$ where $p_a = \mathrm{Pr}(a|s)$.
...
- MCTS details:
    ...
    #### Page 4 / Figure 1: **Self-play reinforcement learning in AlphaGo Zero**
